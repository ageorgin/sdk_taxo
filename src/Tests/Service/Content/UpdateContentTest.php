...
class UpdateContentTest extends \Ftven\SdkTaxonomy\Tests\PHPUnitAbstract
{
    public function testExecute()
    {
        $content = new \Ftven\SdkTaxonomy\Entity\Content('666');
        ...
        $mockGuzzleService = $this->getMock('\Ftven\SdkTaxonomy\Service\GuzzleService', array('put'));
        $mockGuzzleService
            ->expects($this->once())
            ->method('put')
            ->with('/contents/666', ['content-serialize'], ['Content-Type' => 'application/json', 'headers' => 'les-headers'])
        ...
        $svc = new \Ftven\SdkTaxonomy\Service\Content\UpdateContent();
